...
def __compute_and_save_results_to(_filename):
    ...
    def create_poly_3_svm_pca_and_znorm_poly_3_svm_pca_classifiers(pca_dim):
        poly_3_svm_pca_classifiers = [
            SvmClassifier(
                c,
                kernel=PolynomialKernel(3),
        ...
        return poly_3_svm_pca_classifiers + znorm_poly_3_svm_pca_classifiers

    poly_3_svm_pca_and_znorm_classifiers = flat_map(
        lambda x: x,
        [
            create_poly_3_svm_pca_and_znorm_poly_3_svm_pca_classifiers(pca_dim)
            for pca_dim in pca_dimensions
        ]
    )

    classifiers = poly_3_svm_classifiers + znorm_poly_3_svm_classifiers + poly_3_svm_pca_and_znorm_classifiers
...
def __load_and_plot_results_from(_filename, save_plot_into=None):
    ...
    # no PCA
    poly_3_svm_minDCFs = list(map(lambda x: x[0], minDCFs[0:len(regularization_param_values)]))
    znorm_poly_3_svm_minDCFs = list(map(
        lambda x: x[0],
        minDCFs[len(regularization_param_values):2*len(regularization_param_values)])
    )
    plt.plot(regularization_param_values, poly_3_svm_minDCFs, label="SVM - Poly(3)")
    plt.plot(regularization_param_values, znorm_poly_3_svm_minDCFs, label="SVM - Poly(3) (Z-norm)")

    for i,pca_dim in enumerate(pca_dimensions):
        i*=2
        # retrieve minDCFs
        poly_3_svm_pca_minDCFs = list(map(
            lambda x: x[0],
        ...
        # plot curves
        plt.plot(regularization_param_values, poly_3_svm_pca_minDCFs,
                 label="SVM - Poly(3) (PCA=%d)" % pca_dim)
    ...
    plt.xlabel("C")
    plt.ylabel("min DCF")
    plt.title("5-fold CV - 3D Polynomial SVM - PCA + Z-norm effect")
    plt.grid()
...
if __name__ == "__main__":
    ...
    filename = "4_model_selection/poly_3_svm_znorm_PCA_minDCF_3"
    ...
    # print results
    __load_and_plot_results_from(filename, save_plot_into="poly_3_svm_znorm_PCA_minDCF_3.pdf")
