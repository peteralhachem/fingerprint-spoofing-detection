...
def __compute_and_save_results_to(_filename):
    ...
    def create_lr_pca_and_znorm_lr_pca_classifiers(pca_dim):
        lr_pca_classifiers = [
            BinaryLogisticRegressionClassifier(
                regularization_param=_lambda,
        ...
        return lr_pca_classifiers + znorm_lr_pca_classifiers

    lr_pca_and_znorm_classifiers = flat_map(
        lambda x: x,
        [
            create_lr_pca_and_znorm_lr_pca_classifiers(pca_dim)
            for pca_dim in pca_dimensions
        ]
    )

    classifiers = lr_classifiers + znorm_lr_classifiers + lr_pca_and_znorm_classifiers
...
def __load_and_plot_results_from(_filename, save_plot_into=None):
    ...
    for i,pca_dim in enumerate(pca_dimensions):
        i*=2
        # retrieve minDCFs
        lr_pca_minDCFs = list(map(lambda x: x[0], minDCFs[(i+2)*len(lambdas):(i+3)*len(lambdas)]))
        znorm_lr_pca_minDCFs = list(map(lambda x: x[0], minDCFs[(i+3)*len(lambdas):(i+4)*len(lambdas)]))

        # plot curves
        plt.plot(lambdas, lr_pca_minDCFs, label="Log-Reg (PCA=%d)" % pca_dim)
        plt.plot(lambdas, znorm_lr_pca_minDCFs, label="Log-Reg (PCA=%d, Z-norm)" % pca_dim)

    plt.xscale("log")
    plt.xlabel("$\\lambda$")
    plt.ylabel("min DCF")
    plt.title("5-fold CV - Logistic Regression - PCA + Z-norm effect")
    plt.grid()
...
if __name__ == "__main__":
    ...
    filename = "4_model_selection/lr_znorm_PCA_minDCF"
    ...
    # print results
    __load_and_plot_results_from(filename, save_plot_into="lr_znorm_PCA_minDCF.pdf")
