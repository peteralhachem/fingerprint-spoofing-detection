...
def __compute_and_save_results_to(_filename):
    ...
    classifiers = rbf_svm_classifiers
    ...
    for i,pca_dim in enumerate(pca_dimensions):
        ...
        for j,gamma in enumerate(rbf_gamma_param_values):
            rbf_svm_pca_minDCFs = list(map(
                lambda x: x[0],
                minDCFs[(i*l) + j*len(regularization_param_values): (i*l) + (j+1)*len(regularization_param_values)])
            )
...
if __name__ == "__main__":
    ...
    #   without z-normalization preprocessing, for PCA=8 *

    # specify the name of the file which will contain the results inside output/
    filename = "4_model_selection/rbf_3_svm_PCA_8_minDCF"
    ...
    # print results
    __load_and_plot_results_from(filename, save_plot_into="rbf_3_svm_PCA_8_minDCF.pdf")
