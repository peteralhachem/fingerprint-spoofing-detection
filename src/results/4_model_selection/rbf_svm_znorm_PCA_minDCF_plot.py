...
def __compute_and_save_results_to(_filename):
    ...
    def create_rbf_svm_pca_and_znorm_rbf_svm_pca_classifiers(pca_dim):
        ...
        return rbf_svm_pca_classifiers + znorm_rbf_svm_pca_classifiers

    rbf_svm_pca_and_znorm_classifiers = flat_map(
        lambda x: x,
        [
            create_rbf_svm_pca_and_znorm_rbf_svm_pca_classifiers(pca_dim)
            for pca_dim in pca_dimensions
        ]
    )

    classifiers = rbf_svm_classifiers + znorm_rbf_svm_classifiers + rbf_svm_pca_and_znorm_classifiers
...
def __load_and_plot_results_from(_filename, save_plot_into=None):
    ...
    for i, gamma in enumerate(rbf_gamma_param_values):
        ...
        znorm_rbf_svm_minDCFs = list(map(
            lambda x: x[0],
            minDCFs[l+i*len(regularization_param_values): l+(i+1)*len(regularization_param_values)])
        )
        plt.plot(regularization_param_values, znorm_rbf_svm_minDCFs,
                 label="SVM - RBF ($\\log{\\gamma}$ = %d) (Z-norm)" % (math.log10(gamma)))
    ...
    for i,pca_dim in enumerate(pca_dimensions):
        ...
        for j,gamma in enumerate(rbf_gamma_param_values):
            rbf_svm_pca_minDCFs = list(map(
                lambda x: x[0],
                minDCFs[((2*i+2)*l) + j*len(regularization_param_values):((2*i+2)*l) + (j+1)*len(
                    regularization_param_values)])
            )
        ...
        for j, gamma in enumerate(rbf_gamma_param_values):
            znorm_rbf_svm_pca_minDCFs = list(map(
                lambda x: x[0],
                minDCFs[((2*i+3)*l) + j*len(regularization_param_values):((2*i+3)*l) + (j+1)*len(
                    regularization_param_values)])
            )
            plt.plot(regularization_param_values, znorm_rbf_svm_pca_minDCFs,
                 label="SVM - RBF ($\\log{\\gamma}$ = %d) (PCA=%d, Z-norm)" % (math.log10(gamma), pca_dim))

    plt.xscale("log")
    plt.xlabel("C")
    plt.ylabel("min DCF")
    plt.title("5-fold CV - RBF SVM - PCA + Z-norm effect")
    plt.grid()
...
if __name__ == "__main__":
    # * SVM classifiers with Radial Basis Function (RBF) kernel, with different C and gamma
    #   hyperparameter values, with (or without) z-normalization preprocessing, for different values of PCA *

    # specify the name of the file which will contain the results inside output/
    filename = "4_model_selection/rbf_svm_znorm_PCA_minDCF"
    ...
    __load_and_plot_results_from(
        filename,
        save_plot_into="rbf_svm_znorm_PCA_minDCF.pdf"
    )
